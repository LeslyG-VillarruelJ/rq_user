...
import ec.edu.epn.rq_user.api.RetrofitInstance
...
class RutaViewModel : ViewModel() {
    ...
    fun crearRuta(ruta: Ruta) {
        viewModelScope.launch {
            try {
                val rutaCreada = RetrofitInstance.rutaApi.createRoute(ruta)
    ...
    fun obtenerRutas(driverId: String) {
        viewModelScope.launch {
            try {
                ...
                val rutasObtenidas = rutaApi.getRoutes() // Pasamos el driverId en la URL
